...
export function Button({ variant = 'outline', className = '', ...rest }: Props) {
  return (
    <button
      className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all duration-150 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none active:scale-[0.98] ${VARIANTS[variant]} ${className}`}
      {...rest}
    />
  );
}
